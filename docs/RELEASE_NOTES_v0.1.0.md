...
The hardcoded Neo4j password (`"password"`) was removed from [`graph_retriever.py`](src/graph/graph_retriever.py). The system now requires the `NEO4J_PASSWORD` environment variable; startup fails with a `ValueError` if Neo4j is enabled but the password is empty.

**Files Modified:**
- `tests/test_graph_retriever.py` — updated test assertion to expect empty string default

**Evidence:** `grep -r '"password"' src/graph/` → 0 hits
...
**Files Modified:**
- `src/api/rag_server.py` — `allow_origins=CORS_ORIGINS`
- `.env.example` — added `CORS_ORIGINS=http://localhost:3000,http://localhost:5173`
- `tests/test_rag_server.py` — added `TestCORSWhitelist` class (2 tests)
...
All 14 bare `except Exception as e:` blocks in [`rag_server.py`](src/api/rag_server.py) were replaced with specific exception types (`QdrantAPIException`, `ImportError`, `RuntimeError`, `OSError`, `ConnectionError`, `KeyError`, `IndexError`, `yaml.YAMLError`). A global exception handler `@app.exception_handler(Exception)` was added that logs with `logger.exception` and returns sanitized 500 JSON responses.

**Files Modified:**
- `src/api/rag_server.py` — narrowed handlers, added global exception handler
- `tests/test_rag_server.py` — added `TestErrorPathStatusCodes` class (3 tests)

**Evidence:** `grep -r 'except Exception' src/api/rag_server.py` → 0 hits
...
- `src/api/health_check.py` — `requests` → `httpx.AsyncClient`
- `src/api/rag_server.py` — wrapped Qdrant `upsert()` in `asyncio.to_thread`
- `tests/test_health_check.py` — updated mocks for `httpx.AsyncClient`
...
- `pytest.ini` — added `pythonpath = src`, added `optional` marker
- `src/multimodal/unified_retriever.py` — fixed relative import
...
Removed 334 lines of dead code: `ContextMemory` class (137 lines) and `SessionMemory` class (197 lines) from [`memory_manager.py`](src/core/memory_manager.py). Aligned `MemoryConfig.embedding_model` default from `"sentence-transformers/all-MiniLM-L6-v2"` to `nomic-ai/nomic-embed-text-v1.5`. Line count: 747 → 382 LOC.

**Files Modified:**
- `src/core/memory_manager.py` — deleted ContextMemory, SessionMemory, cleaned references
- `README.md` — added release notes entry

**Evidence:**
- `rg 'class ContextMemory|class SessionMemory' src/` → 0 hits
- `rg 'get_context_memory|get_session_memory' src/` → 0 hits
- `wc -l src/core/memory_manager.py` → 382 lines
- Reviewer Agent: **PASS** ([TASK-040__REVIEW_REPORT.md](memory/TASKS/TASK-040__REVIEW_REPORT.md))
...
**Files Modified:**
- `src/core/memory_persistence.py`
- `tests/test_crash_stress.py`
...
- `src/api/rate_limiter.py` — new module
- `src/api/rag_server.py` — integrated limiter
- `config/default.yaml` — rate limit config
- `.env.example` — environment variable docs
- `tests/test_rate_limiter.py` — new test suite
...
- `src/api/scanner_manager.py` — new module
- `src/api/rag_server.py` — integrated scanner lifecycle
- `tests/test_scanner_integration.py` — new integration tests
...
**Files Created/Modified:**
- `src/api/health_check.py` — new module
- `src/api/rag_server.py` — integrated health endpoints
- `tests/test_health_check.py` — new test suite
